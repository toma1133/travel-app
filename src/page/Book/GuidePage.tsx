...
const GuidePage = ({ isPrinting }: CoverPageProps) => {
    ...
    // Create & edit
    const initialPlaceState: PlaceVM = {
        created_at: null,
        description: "",
        eng_name: "",
    ...
    const handleSubmit = (e: FormEvent) => {
        ...
        if (modalMode === "create") {
            insert.mutate({ ...placeData, id: crypto.randomUUID() });
        } else {
            upsert.mutate(placeData);
        }
    ...
    const handleConfirmDelete = () => {
        if (placeToDelete) {
            remove.mutate(placeToDelete.id);
        }
        ...
        <div
            className={`min-h-full font-[Noto_Sans_TC] text-gray-800 ${
                isPrinting
                    ...
                    : `${tripData?.theme_config?.bg || "bg-gray-100"} pt-12 pb-24`
            }`}
        >
            {!isPrinting && (
                <SectionHeader
                ...
                <div className="flex justify-between items-center px-4 mb-6">
                    <div className="flex space-x-2 overflow-x-auto no-scrollbar">
                        {placeCategories.map((f) => (
                            <button
                                key={f.id}
                                onClick={() => handleFilterBtnClick(f.id)}
                                className={`px-4 py-1.5 rounded-full text-xs transition-colors ${
                                    filter === f.id
                                        ? `${tripData?.theme_config?.accent} text-white`
                    ...
                    <button
                        ...
                        className={`ml-2 flex px-3 py-1.5 rounded-full text-xs font-medium text-white shadow-sm ${tripData?.theme_config?.accent} hover:opacity-90 transition-opacity whitespace-nowrap`}
                    >
