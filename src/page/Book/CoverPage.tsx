...
const CoverPage = () => {
    ...
    const { data: tripData, isLoading, error } = useBook(id);

    return (
        <div className={`relative h-full flex flex-col bg-[#2C3E50]`}>
            ...
            <div className="flex-1 px-8 -mt-20 relative z-10">
                <div className="bg-[#F9F8F6] p-8 shadow-2xl rounded-t-sm h-full flex flex-col relative">
                    ...
                    <div className="mt-4 text-center">
                        <span
                            className={`inline-block px-3 py-1 ${tripData?.theme_config.accent} text-white text-[10px] tracking-widest mb-4`}
                        >
                            TRAVEL ITINERARY
                        </span>
                        <h1
                            className={`text-3xl font-[Noto_Sans_TC] font-bold ${tripData?.theme_config.primary} mb-2`}
                        >
                            {tripData.title}
                        </h1>
                        <p
                            className={`text-sm ${tripData?.theme_config.subtle} font-medium mb-6 uppercase tracking-widest`}
                        >
                            {tripData.subtitle}
                        </p>
                    </div>
                    ...
                    <p
                        className={`text-sm ${tripData?.theme_config.primary} leading-loose text-justify font-light opacity-90`}
                    >
